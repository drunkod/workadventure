<script lang="ts">
    ...
    function getSsoUrl() {
        if (!authSessionId) {
            throw new Error("This UIA flow requires an authSessionId");
        }
        return matrixClient.getFallbackAuthUrl(AuthType.Sso, authSessionId);
    }
    ...
    const onStartAuthClick = () => {
        popupWindow = window.open(getSsoUrl(), "_blank");
