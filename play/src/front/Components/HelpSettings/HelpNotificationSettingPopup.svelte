...
>
    ...
    <section class="flex row justify-center p-4 bg-contrast">
        <button class="btn bg-white/10 hover:bg-white/20 mr-2 w-full justify-center" onclick={preventDefault(refresh)}
            >{$LL.notification.help.refresh()}</button
        >
        <button type="submit" class="btn btn-danger w-full justify-center" onclick={preventDefault(close)}
