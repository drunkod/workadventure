<script lang="ts">
    ...
    let { restrictedRightsPropertyData = $bindable() }: Props = $props();

    let writeTags: InputTagOption[] | undefined = $state(restrictedRightsPropertyData.writeTags.map((writeTag) => ({
        value: writeTag,
        label: writeTag,
        created: false,
    })));
    let readTags: InputTagOption[] | undefined = $state(restrictedRightsPropertyData.readTags.map((readTag) => ({
        value: readTag,
        label: readTag,
        created: false,
    })));
